...
export default function ColorPicker({ color, onChange, label }: ColorPickerProps) {
  ...
  const safeHex = /^#[0-9a-fA-F]{6}$/.test(color) ? color : '#d9d9d9'
  const [h0,s0,v0] = rgbToHsv(...hexToRgb(safeHex))
  const [hue, setHue] = useState(h0)
  const [sat, setSat] = useState(s0)
  const [val, setVal] = useState(v0)
  const [hexIn, setHexIn] = useState(safeHex.slice(1).toUpperCase())

  useEffect(() => {
    const h = /^#[0-9a-fA-F]{6}$/.test(color) ? color : '#d9d9d9'
  ...
  const commit = (h: number, s: number, v: number) => {
    ...
    onChange(hex)
    setHexIn(hex.slice(1).toUpperCase())
  }
  ...
  const svDrag = mkDrag(e => {
    ...
    setSat(x); setVal(1-y); commit(hue,x,1-y)
  })
  ...
  const hueDrag = mkDrag(e => {
    ...
    setHue(x*360); commit(x*360,sat,val)
  })
  ...
  const hexSubmit = () => {
    ...
    else setHexIn(safeHex.slice(1).toUpperCase())
  }
  ...
  return (
    <div style={{position:'relative',display:'flex',alignItems:'center',gap:6}}>
      <div ref={dotRef} onClick={openPicker} className="cdot" style={{width:14,height:14,borderRadius:6,background:safeHex,border:'1px solid rgba(255,255,255,0.15)',cursor:'pointer',flexShrink:0}}/>
          ...
          <div style={{display:'flex',gap:6,alignItems:'center'}}>
            <div className="hex-input" style={{display:'flex',alignItems:'center',background:'#3a3a3a',borderRadius:6,border:'1px solid #555',flex:1,overflow:'hidden'}}>
              ...
              <input value={hexIn}
                ...
                style={{background:'transparent',border:'none',outline:'none',color:'#eee',fontSize:12,padding:'0 8px',width:62,fontFamily:'monospace'}}
                maxLength={6}/>
            </div>
            <button
              onClick={async () => {
